...
public class SearchByArtistService {
	...
	public static String d(String paramString) {
		...
		int i = 0;
		while ( i < k ) {
			localStringBuilder.append( a( "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(
					bytes[i] ), 5 ) );
			i += 1;
		}
		...
		i = j;
		while ( i < k ) {
			builder.append( "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".charAt( Integer.parseInt(
					localStringBuilder.substring( i, i + 6 ),
					2
			) ) );
			i += 6;
		}
	...
	public String getToken() {
		...
		JsonNode node = requestTemplate.getForObject(
				"https://api.zaycev.net/external/auth?code={code}&hash={hash}",
				JsonNode.class,
	...
	public final String b(String paramString) {
		...
		while ( i < digest.length ) {
			for ( paramString = Integer.toHexString( digest[i] & 0xFF );
